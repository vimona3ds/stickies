...
export function getGameResults(state: GameState): GameResults {
  ...
  const startTime = (status === GameStatus.RESULTS || status === GameStatus.PLAYING) ? state.startTime : endTime;
  const speed = (endTime - startTime) / 1000;
  const mistakes = (status === GameStatus.RESULTS || status === GameStatus.PLAYING) ? state.mistakes : 0;
